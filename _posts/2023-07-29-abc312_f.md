...
impl SortedMultiSet {
    ...
    fn add(&mut self, v: usize) {
        self.st.insert(v);
        let c = self.cnt.get(&v).unwrap_or(&0usize).to_owned();
    ...
    fn vmin(&self) -> usize {
        self.st.iter().next().unwrap().to_owned()
    }

    fn vmax(&self) -> usize {
        self.st.iter().last().unwrap().to_owned()
    }
    fn len(&self) -> usize {
        self.num
    }
    fn sum(&self) -> usize {
        ...
        for k in self.cnt.keys() {
            ret += self.cnt.get(k).unwrap().to_owned() * k;
        }
        ret
    }
}
